...
class CarImg:
    ...
    def get_img(self,url_):
        for i in range(2,455):
            ...
            tree = etree.HTML(r)
            imgs_url = set(tree.xpath('//div[@class="row block-4col-180"]//img//@src'))
            if imgs_url is not ():
                for item in imgs_url:
                    img = requests.get(item, headers=self.headers)
                    with open(f'/Volumes/Don_disk/car_img/{self.count}.png', 'wb+') as f:
                        self.count += 1
                        f.write(img.content)
            if self.count % 100 == 0:
...
if __name__ == '__main__':
    car = CarImg()
    car.get_img('https://photo.yiche.com/')
